...
@app.post("/recommend/save/{user_id}", tags=["Recommend"])
async def save_recommendation(user_id: UUID, request: RecommendSaveRequest):
    ...
    try:
        from model_ai.recommend.utils.database import db_manager
...
@app.get("/recommend/history/{user_id}", tags=["Recommend"])
...
@app.get("/recommend/plan/{plan_id}", tags=["Recommend"])
...
@app.delete("/recommend/plan/{plan_id}", tags=["Recommend"])
